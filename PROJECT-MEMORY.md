...
| **Last completed** | **M0-12 R2 signing, 2026-09-16 (OpenAI Codex).** Added strict server-generated key builders/parser, binding-only JSON reads, five-minute `aws4fetch` presigned GETs, attempt/test/version/status enforcement, and the submitted + released transcript gate. `key.json` and `content.json` refuse signing before credentials are read. The CSP admits only this account's exact R2 origin for images/media, while the bundle guard blocks both R2 credential names. 12 focused checks; all 134 unit tests, 250 DB checks, the 26-policy sweep, typecheck, lint, production build, client-bundle scan and npm audit pass. Live object-read credentials remain M0-14 provisioning. Earlier: M0-16 upload schema; M0-15 taxonomy; the complete front end on the view-model/mock seam; `plan_notes`; M0-13 CSP + sanitiser; M0-12 part 1; M0-22 CI; M0-19; M0-11; M0-05…M0-10 (**schema complete, 24 tables**). |
| **Next task — back end** | **Phase 3, the content pipeline**, in order: **M0-17** importer + CLI (step 26) → **M0-18** `lib/scoring.ts` (step 27) → **M0-20** port the legacy Listening test (step 28). Also open: **M0-14** provision scoped R2 object-read credentials, **M0-21** docs/ADRs (step 11; `docs/test-authoring.md` now exists, but the prescribed tree and ADRs do not), the M0-13 leftover (measure per-route Worker CPU against the 10 ms free limit), and M1-01's `supabase/config.toml` `enable_signup` alignment. |
...
| **Open PRs** | **#10 M0-16** → **#11 M0-12**. They are intentionally stacked in that merge order so each later PR sheds shared commits without conflicts. No PR was merged by the agent. |
...
| **Branch** | `feat/m0-12-r2-signing`, stacked on `feat/m0-16-upload-schema` / PR #10 to avoid conflicting rework. |
