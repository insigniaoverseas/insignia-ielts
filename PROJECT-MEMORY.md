...
| **Last completed** | **M0-16 upload schema and authoring guide, 2026-09-16 (OpenAI Codex).** Pinned Zod 4.6.5 and added a strict, inferred `schema_version: 1` contract for Listening, Academic Reading and GT Reading. It validates type/widget/container agreement from M0-15, per-widget answer shapes, format/audio/passage rules, contiguous numbering, practice-set purity, safe filenames and asset references. All issues carry nested field paths. `docs/test-authoring.md` documents every field; its three executable JSON samples cover all 18 canonical types. 16 focused checks; all 122 unit tests, 250 DB checks, the 26-policy sweep, typecheck, lint, production build, client-bundle scan and npm audit pass. Earlier: M0-15 taxonomy; the complete front end on the view-model/mock seam; `plan_notes`; M0-13 CSP + sanitiser; M0-12 part 1; M0-22 CI; M0-19; M0-11; M0-05…M0-10 (**schema complete, 24 tables**). |
| **Next task — back end** | **Phase 3, the content pipeline**, in order: **M0-12 part 2** `lib/r2.ts` signing (step 25) → **M0-17** importer + CLI (step 26) → **M0-18** `lib/scoring.ts` (step 27) → **M0-20** port the legacy Listening test (step 28). Also open: **M0-21** docs/ADRs (step 11; `docs/test-authoring.md` now exists, but the prescribed tree and ADRs do not), the M0-13 leftover (measure per-route Worker CPU against the 10 ms free limit), and M1-01's `supabase/config.toml` `enable_signup` alignment. |
...
| **Open PRs** | None. #9 (M0-15) is merged; M0-16 is on local branch `feat/m0-16-upload-schema`. |
...
| **Branch** | `feat/m0-16-upload-schema`, branched from `main` after merged PR #9. |
