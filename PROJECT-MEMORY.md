...
| **Last completed** | **M0-15 question-type taxonomy, 2026-09-16 (OpenAI Codex).** `lib/question-types.ts` is now the single source of truth for all 18 canonical types, six widgets, completion containers and Listening/Academic/GT availability. GT's two official-list omissions return `warning`, not a boolean or hard failure. The player view-model imports its widget/container vocabulary from this module instead of duplicating it. 25 focused checks; all 106 unit tests, 250 DB checks, the 26-policy sweep, typecheck, lint, production build and client-bundle scan pass. **The whole front end** was completed earlier the same day: all 30 screens on the `lib/view-models/*` ↔ `lib/mock/*` seam. Earlier back end: the `plan_notes` migration (Q12, M0-07), M0-13 CSP + sanitiser, M0-12 part 1, M0-22 CI, M0-19, M0-11, M0-05…M0-10 (**schema complete, 24 tables**), M0-03, M0-01. |
| **Next task — back end** | **Phase 3, the content pipeline**, in order: **M0-16** upload schema in Zod (step 24, needs the `zod` dependency) → **M0-12 part 2** `lib/r2.ts` signing (step 25, parallel-safe) → **M0-17** importer + CLI (step 26) → **M0-18** `lib/scoring.ts` (step 27) → **M0-20** port the legacy Listening test (step 28) → `docs/test-authoring.md` (step 30). Also open: **M0-21** docs/ADRs (step 11, `docs/` does not exist yet), the M0-13 leftover (measure per-route Worker CPU against the 10 ms free limit), and M1-01's `supabase/config.toml` `enable_signup` alignment. |
...
| **Open PRs** | None. #6 and #7 are merged; `main` contains the student, admin, teacher and auth screens. |
