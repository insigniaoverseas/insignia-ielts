...
**Start with.** If the user has run `db push`: verify over MCP (`list_tables`, advisors — expect none), `npm run db:types`, commit the types, close M0-06. Then M0-07 (cohorts + `is_teacher_of` + teacher policy on `users` + `invitations.batch_id` FK). Q11 answered: Workers Free. Open: whether to commit the PGlite harness as the M0-11 test; the Worker's workers.dev URL (not in the repo, and wrangler can't print it) — needed to measure real CPU per request with `wrangler tail`.
