...
| **Last completed** | **M0-17 importer + CLI, 2026-09-16 (OpenAI Codex).** One `importTest()` path now validates, sanitises, checks file MIME/magic/size and MP3 mono/bitrate, generates all R2 keys, splits protected scoring fields into `key.json`, uploads private objects and then creates an attributed draft. Failure attempts reverse-order object rollback. The CLI has write-free dry-run and remote audited import modes. A recursive test proves `content.json` contains none of the four protected keys. 8 focused checks; all 142 unit tests, 250 DB checks, the 26-policy sweep, typecheck, lint, production build, client-bundle scan and npm audit pass. Earlier: M0-12 R2 signing; M0-16 upload schema; M0-15 taxonomy; the complete front end; `plan_notes`; M0-13 CSP + sanitiser; M0-22 CI; M0-19; M0-11; M0-05…M0-10 (**schema complete, 25 tables**). |
| **Next task — back end** | **Phase 3, the content pipeline**, in order: **M0-18** `lib/scoring.ts` (step 27) → **M0-20** port the legacy Listening test (step 28). Also open: **M0-14** provision scoped R2 object-read credentials, **M0-21** docs/ADRs (step 11; `docs/test-authoring.md` now exists, but the prescribed tree and ADRs do not), the M0-13 leftover (measure per-route Worker CPU against the 10 ms free limit), and M1-01's `supabase/config.toml` `enable_signup` alignment. |
...
| **Open PRs** | **#10 M0-16** → **#11 M0-12** → **#12 M0-17**. They are intentionally stacked in that merge order so each later PR sheds shared commits without conflicts. No PR was merged by the agent. |
...
| **Branch** | `feat/m0-17-test-importer`, stacked on M0-12 / PR #11 (which is stacked on M0-16 / PR #10) to avoid conflicting rework. |
