...
| **Open PRs** | M0-20 converter PR pending from `feat/m0-20-legacy-listening-test`. #10 M0-16, #11 M0-12, #12 M0-17 and #13 M0-18 are merged. The agent does not merge this work. |
...
| **Blocked on** | M0-20's external completion inputs: a real mono ≤64 kbps MP3, its exact four contiguous section-end timestamps, and an active user holding `test:author`. Repository/history/live mock contain no recording; the live `tests` table and eligible-author query are both empty as of 2026-09-16. |
...
| M0-20 Port legacy Listening test | in_progress | OpenAI Codex | 2026-09-16 | ✅ Strict converter maps all 40 source controls/answers to canonical upload JSON; 8 focused checks prove valid type mapping, required real timings, the CLI handoff, and a 40-entry private key with zero scoring fields in content. ✅ Legacy CLI delegates to the canonical importer; its end-to-end test caught and fixed the existing unbound `crypto.randomUUID` CLI bug. ✅ Unit 172/172, DB 250/250, policy sweep 26/26, typecheck, lint, build, bundle scan and audit pass. ⬜ Real MP3 + four section ends + active `test:author`; then dry-run and audited draft import. |
...
| **Legacy answer keys** | The checked-in prototype `ielts-data.js` has 40 non-empty answers and they pass structural/choice-bank validation. It has no recording or transcript, so the answers are not semantically verified against audio; keep the imported test in draft until a human listens and checks every key. |
