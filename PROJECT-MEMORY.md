...
| **Next task** | **M0-07** cohorts & plans migration ([`BUILD-STEPS.md`](BUILD-STEPS.md) step 15): `batches`, `batch_teachers`, `batch_students`, `student_plans`, `plan_history` + `private.is_teacher_of()` + teacher policy on `users` + `invitations.batch_id` FK + one combined SELECT policy per identity table (advisor). Same workflow: agent writes + PGlite + dry-run, user reviews and pushes. No-DB tasks that can run alongside: M0-12 part 1 (R2 buckets), M0-13 (CSP), M0-15 (`question-types.ts`), M0-18 (`scoring.ts`), M0-21 (docs). |
...
**Start with.** M0-07 (cohorts + `is_teacher_of` + teacher policy on `users` + `invitations.batch_id` FK). Q11 answered: Workers Free. Open: whether to commit the PGlite harness as the M0-11 test; the Worker's workers.dev URL (not in the repo, and wrangler can't print it) — needed to measure real CPU per request with `wrangler tail`.
