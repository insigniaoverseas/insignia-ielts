...
| M0-12 R2 private buckets + `lib/r2.ts` signing | in_progress | Claude, goverdhan-gaur | 2026-09-15 | ✅ Part 1: `insignia-ielts-content` + `insignia-ielts-audio` created (APAC, user approved), r2.dev public URL disabled (verified), bound as `CONTENT_BUCKET` / `AUDIO_BUCKET`, `cf-typegen` regenerated, `wrangler deploy --dry-run` shows both. ⬜ Part 2: `lib/r2.ts` signing rules (step 25). `key.json` never signable. |
| M0-13 Security headers + nonce CSP + sanitize | in_progress | Claude, goverdhan-gaur | 2026-09-15 | ✅ Nonce policy chosen by the user. `src/proxy.ts`, `src/lib/security/headers.ts` (+README), `public/_headers` (OpenNext's cache rule kept), root layout `await connection()` → all routes dynamic. ✅ `test:unit` 41/41 (12 header checks incl. `_headers` ↔ `headers.ts` sync). ✅ Local workerd + headless Chrome: injected inline script **blocked**, React hydrates, 0 violations on `/` and `/dev/components`; static chunks carry cache + security headers. ✅ **Verified on a real Cloudflare preview** (branch `feat/security-headers-csp`, Workers Builds preview version `6f5a70dc`): all headers present, fresh nonce per request, 12/12 scripts carry it, static chunks keep the cache rule, headless Chrome: React hydrates, 0 CSP violations on `/` and `/dev/components`; live site unchanged. ⬜ Merge the PR (deploys). ⬜ `lib/security/sanitize.ts` (rehype-sanitize allowlist). ⬜ Measure page CPU vs the 10 ms free limit now that every page is dynamic. |
...
| **`gh` can't open PRs; git can push** | `gh` is logged in as `goverdhan-gaur`, which has **read-only** access to `insigniaoverseas/insignia-ielts`; git pushes with a different, write-enabled credential. So the agent can push branches but `gh pr create` fails ("must be a collaborator"). Fix: give `goverdhan-gaur` write access, or `gh auth login` with the org account. Until then the user opens PRs. | M0-13 |
