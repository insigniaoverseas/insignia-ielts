...
| | |
|---|---|
| **Active milestone** | **Supabase application wiring.** Every production student, teacher and admin read now comes from the signed-in user's RLS-scoped Supabase client. Route-family role access is enforced in `proxy.ts` and repeated in server layouts; individual staff screens retain permission checks. Private Cloudflare R2 test content, audio and answer keys remain deliberately deferred. |
...
| **Branch** | `feat/supabase-rbac-data`, created fresh from local `main` at `3117e5e`. It has no upstream until explicitly pushed with `git push -u origin feat/supabase-rbac-data`. |
...
| M0-17 Importer: validate → split → upload | done | OpenAI Codex | 2026-09-16 | ✅ One typed dependency-injected path for CLI/admin/MCP. ✅ Strict validation + authored-HTML sanitising; 5 MiB image / 20 MiB MP3 caps, MIME + magic checks, mono and ≤64 kbps. ✅ Generated UUID/version keys; source file names never become object keys. ✅ Protected fields exist only in `key.json`; transcript separate. ✅ Objects upload before the draft row; rollback attempted on failure. ✅ CLI dry-run/remote modes, active `test:author` actor gate and audit row. 8 focused checks; unit 142/142 and all gates pass. ✅ **Admin upload screen (2026-09-17)**: `/admin/library/new` — screen 26's "Add test" button was a dead link, so a test could only enter the catalogue from a terminal with the R2 credentials to hand. Two steps on one form: **Check** validates and prints every R2 key it would write without writing anything, **Import** runs the same `importTest` pipeline through the Worker bindings. `next.config.ts` raises `serverActions.bodySizeLimit` to 32 MB — the 1 MB default refuses a Listening MP3 before any of our code runs. ⬜ Anything past ~32 MB needs a presigned direct-to-R2 PUT (M8-04). |
...
| M1-04 SPF / DKIM / DMARC | done | goverdhan-gaur, Claude Opus 5 | 2026-09-17 | ✅ **Verified in public DNS 2026-09-17**, not just assumed. Sending domain is `app.insigniaoverseas.com` (a subdomain, so the root's iCloud mail reputation is untouched). ✅ **DKIM**: `resend._domainkey.app.insigniaoverseas.com` publishes a direct RSA TXT. ✅ **SPF**: `send.app.insigniaoverseas.com` is a CNAME to `send.forge.rmta.net`, which publishes `v=spf1 ip4:52.3.252.119 ip4:44.222.39.36 ip4:199.249.231.0/24 ~all`. ✅ **Bounce MX**: `send.app…` → `feedback.forge.rmta.net`. ✅ **DMARC**: no subdomain record, so it inherits the org domain's `v=DMARC1; p=quarantine; adkim=r; aspf=r` — a real enforcing policy. **Both mechanisms align**: DKIM `d=` matches the From domain exactly, and the envelope sender on `send.app…` shares the org domain with the From domain, which relaxed alignment accepts. ⬜ Optional: `rua` points at `dmarc_rua@onsecureserver.net`, a registrar default nobody reads — repoint it to a real mailbox to actually see failures. ⬜ Deliverability at *volume* is untested: `bulkInviteAction` can send 40 at once from a subdomain with no sending history. |
...
| M1-11 Turnstile on login + accept-invite | todo | | | ✅ **Unblocked 2026-09-17**: both keys are present in `.dev.vars`, so this is now code work, not a credentials wait. Names are in `.dev.vars.example`; the sign-in action is the single place it hooks into. |
| M1-12 Session cookies + single active session | in_progress | Claude | 2026-09-17 | ✅ `lib/auth/sessions.ts`. A JWT cannot express revocation and its lifetime is deliberately longer than the longest test, so the `insignia_session` cookie carries a `user_sessions.id` and every guarded page checks that row is live. ✅ **Students one session, staff several** (user, 2026-09-17, §4): a student's second sign-in revokes the first — the sharing control — while staff work on a phone and a laptop. ✅ `revokeSession` checks ownership, so one user cannot revoke another's. ✅ **A missing session cookie is signed out** (2026-09-17): it used to count as live so a deploy would not evict anyone, but that let a browser skip revocation by deleting one cookie. ✅ `startSession` failing now fails the sign-in and drops the JWT — a JWT with no revocable session is only half a login. ✅ `Secure` is set from `x-forwarded-proto`, not `NODE_ENV`, so a local HTTP session cookie survives the next request. ✅ `touchSession` writes at most once per 5 minutes. ✅ **Log out actually logs out** (2026-09-17): `signOutAction` was written in M1 but never wired — the Profile button's `onConfirm` was still the pre-M1 no-op that only closed the dialog, so nothing revoked the session. Staff had no log-out control at all; `StaffSidebar` now ends with one. ⬜ Raise the Supabase JWT expiry to 7200 s (needed: a 60-minute Reading test outlasts the 3600 s default and would refresh mid-test) — a dashboard change, awaiting the user. |
...
| M2-02 Student Home (03) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ `/home` retains its designed states, but `getStudentHome()` now reads only RLS-scoped Supabase rows. |
| M2-03 My Tests (04) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Assignment, attempt, unlock, score and practice-test data now comes from Supabase. Locked cards state the computed reason. |
...
| Task | Status | Owner | Date | Note |
|---|---|---|---|---|
| M4-01 Review my mistakes (10) + release gating | in_progress | Claude, OpenAI Codex | 2026-09-17 | ✅ Supabase server-side ownership, result-release and `allow_review` gates. ⬜ Questions/explanations remain private R2 content; the page states that dependency rather than rendering fixture answers. |
| M4-02 Transcript + jump to timestamp | todo | | | Offsets into the one audio file |
| M4-03 My Progress (11) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Released Supabase scores drive band-over-time, tests taken, averages and weakest question types. No invented points. |
| M4-04 Practice at home (12) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Published Supabase practice tests and the student's own attempt counts, with Listening/Reading URL filters. |
| M4-05 Practice instant feedback round-trip | todo | | | One verdict, never the key |
| M4-06 Profile (13) + device management | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Supabase identity, branch, batch, plan and `user_sessions` rows. Device-revocation controls remain a separate mutation task. |

### M5 — Admin essentials

| Task | Status | Owner | Date | Note |
|---|---|---|---|---|
| M5-01 Admin shell + sidebar | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Admin/Owner route access is enforced in both the request proxy and the server layout; individual pages also check their required permission. |
| M5-02 Admin overview (20) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ `/admin/overview` stats, expiring students and audit activity are Supabase-backed. |
| M5-03 Students list (21) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ `/admin/students` reads Supabase users, plans, memberships, attempts, scores and sessions; search/status/batch filters stay URL-addressable. ⬜ Bulk mutations remain separate work. |
...
| M5-05 Student detail drawer (23) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ `/admin/students/[id]` remains a linkable page; student, plan history, attempts and audit rows now come from Supabase. |
...
| Task | Status | Owner | Date | Note |
|---|---|---|---|---|
| M6-01 Teacher dashboard (14) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Assembled from the signed-in teacher's RLS-visible Supabase batches, assignments, attempts, scores and plans. |
| M6-02 Batch view (15) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Roster, plan expiry and activity are Supabase-backed. |
| M6-03 Assign a test (16) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Assignment options now come from Supabase-visible batches, students and published tests. ⬜ Creating the assignment remains a server-action task. |
| M6-04 Results & release (18) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Reads Supabase attempts, scores and answer marks. Correct-answer content is omitted until private R2 is connected; release remains a separate mutation. |
...
| M6-06 Class analytics (19) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Band distribution, weakest types and most-missed questions derive from Supabase scores and marks. |
...
| M7-02 Live session monitor (17) | done | Claude, OpenAI Codex | 2026-09-17 | ✅ Renders the assignment's Supabase attempts, student names, remaining server time, saved-answer counts and tab-switch flags. ⬜ The 10-second poll itself is M7-01. |
...
|---|---|---|---|---|
| 2026-09-17 | M9-04, M2-02 | **Navigation acknowledges the tap.** There was no `loading.tsx` and no Suspense boundary anywhere, so clicking a link left the *previous* screen on the glass until the next one had fully rendered — which is why ~700 ms read as two seconds, and why it felt jittery rather than merely slow. Five route-group loading states added: `(auth)`, `(student)`, `admin`, `teacher`, `attempt`. Each is shaped like the screen it stands in for, per `components/ui/skeleton.tsx`'s own rule — *blocks matching the final layout, never a spinner* — so nothing jumps when the data lands. The shell lives in the layout above, so moving between two admin pages swaps only the body, immediately. The **player's** state is deliberately not test-shaped and carries words rather than blocks: a student tapping "Start test" must not see a skeleton and think the paper has appeared, so it says *"Your time hasn't started yet"* — the timer is server-owned (non-negotiable 1) and saying so is the difference between waiting and tapping again. `aria-busy` + an `sr-only` "Loading…", since the blocks themselves are `aria-hidden`. Reduced motion is already handled globally in `globals.css`. Uncovered, deliberately: `/` (a redirect) and `/dev/components`. 238 unit checks, typecheck, lint, build and bundle scan pass. | `src/app/{(auth),(student),admin,teacher,attempt}/loading.tsx` | Claude Opus 5 |
| 2026-09-17 | M2-02, M5-07 | **Query waterfalls collapsed; sequential steps are the whole cost.** Measured first: 6 queries **sequentially** take 1.73 s, the same 6 **in parallel** take 0.47 s. Supabase is not throttled — each request costs ~235 ms of fixed latency, of which only ~21 ms is network (TCP connect), the rest being free-plan server overhead. So a wider query is free and an extra *sequential step* costs a quarter-second; the enemy is depth, not query count. Two fixed: `loadStudentContext` was two barriers **and was called twice per page** (`getStudentHome` calls it, then calls `getMyTests`, which called it again) — now one embedded query wrapped in `cache()`, keyed on `userId` alone so that creating the Supabase client inside does not defeat the memoisation. `getBatchDetail` was three barriers (batch → branch/teachers/members → the members' names) and is now one. Verified against live data: the embedded student query returns profile, branch, both batches and the plan in **235 ms warm**, identical to a trivial one-row query. PostgREST needed `student_plans!student_plans_student_id_fkey` spelled out, because that table reaches `users` twice. ~700 ms off `/home`, ~470 ms off a batch page. 238 unit checks, typecheck, lint, build and bundle scan pass. ⬜ Still stacked: `getStudentDetail` (6), `getLiveSession` (6), `getMyTests`, `getBatchView`, `getClassAnalytics`, `getAssignmentResults`. | `src/lib/queries/{student,admin}.ts` | Claude Opus 5 |
| 2026-09-17 | M1-12, M1-13 | **Navigation was 2–3 s because every page made eight sequential auth round trips.** Measured, not guessed: Supabase in ap-south-1 answers in **~230 ms** from the office, and one `/admin/batches` navigation spent **2.71 s** on authentication *before any page data*. The eight: Proxy read `users` then `user_sessions`; the admin layout's `requireUser` read `users`, `user_sessions` and wrote `last_seen_at`; then the **page's** `requireUser` did all three again, because nothing was memoised. Four fixes, no change to either gate: (1) `getActor` and `sessionState` wrapped in React `cache()`, so a layout and the page it wraps cost one set of reads rather than two; (2) `touchSession` moved off the critical path with `waitUntil` — nothing being rendered depends on "last seen"; (3) Proxy's two queries collapsed into one, embedding `user_sessions` filtered to the cookie's row (RLS "Users read their own sessions" keeps it honest); (4) the guard's profile read and session check now run in parallel, which is safe because the JWT subject comes from a **local** verify — the project signs ES256, so `getClaims` never touches the network. **2.71 s → 0.80 s.** 238 unit checks, typecheck, lint, build and bundle scan pass. ⬜ The remaining 0.8 s is three unavoidable-looking round trips; passing the Proxy's findings to the render through request headers would cut one more, but that trades away part of the second gate and was not done unilaterally. | `src/lib/rbac.ts`, `src/lib/auth/{sessions,guard}.ts`, `src/proxy.ts` | Claude Opus 5 |
| 2026-09-17 | M0-17, M0-21 | **A test can be uploaded from the browser, and the R2 layout is written down.** `/admin/library/new` + `lib/actions/tests.ts` + `lib/import/r2-import-target.ts`, driving the existing `importTest` — its doc always said "CLI, admin UI and MCP", and this is the admin UI it meant. Check-then-import, so the author sees every key that would be created while it is still free to be wrong. **The client-bundle guard earned its keep**: the first build failed because the literal `key.json` appeared in the preview's *explanatory text* in a client component. Fixed by keeping the answer-key object out of the payload entirely and describing it instead — the guard was not relaxed. New [`docs/r2-layout.md`](docs/r2-layout.md): every bucket, path, file name, content type and signing rule, taken from `lib/r2-keys.ts` rather than from the spec, so it matches what the code enforces. 238 unit checks, typecheck, lint, build and bundle scan pass; `/admin/library/new` is a registered route and guarded on `test:author`. ⚠️ No test has been uploaded — the user asked for the system, not the content. | `src/app/admin/library/new/page.tsx`, `src/components/admin/upload-test-form.tsx`, `src/lib/actions/tests.ts`, `src/lib/import/r2-import-target.ts`, `next.config.ts`, `docs/r2-layout.md` | Claude Opus 5 |
| 2026-09-17 | M5-07, M2-04 | **Batch membership is editable at any time, and a student can be in more than one batch.** Membership was written *only* by the `accept_invitation` migration — set at the invite and never again — so a student in the wrong batch stayed there. New `lib/batch-membership-plan.ts` (pure, import-free, 12 unit checks) works out the row changes; `addStudentsToBatch` / `removeStudentFromBatch` in `lib/batches.ts` apply them; `components/admin/batch-roster.tsx` is the UI on screen 25b. **Two ways in, chosen by the admin** (user's call, 2026-09-17): *Move* closes their other active memberships — a promotion — and *Also add* leaves them, for a student genuinely taking a second course. Leaving sets `left_at`, never deletes, so a result earned in a batch can still name it. **Reopen, not re-insert**: `batch_students` is keyed `(batch_id, student_id)`, so somebody returning to a batch they left must have their old row reopened or the insert dies on the primary key — the single sharpest edge here, and tested. Multi-batch forced a matching change on the student side: `getStudentHome` took `.limit(1).maybeSingle()` on memberships and `StudentIdentity.batchName` became `batchNames: string[]`, rendered on Profile as "Batch" or "Batches". Audited as `batch.join` / `batch.promote` / `batch.leave`. 238 unit checks, typecheck, lint, build and bundle scan pass. ⚠️ Not yet exercised against live rows — `batch_students` is empty because the one student was invited with `batch_id: null`. | `src/lib/batch-membership-plan.ts`, `src/lib/batches.ts`, `src/lib/actions/batches.ts`, `src/components/admin/batch-roster.tsx`, `src/app/admin/batches/[batchId]/page.tsx`, `src/lib/queries/{batches,student}.ts`, `src/lib/view-models/student.ts`, `src/app/(student)/profile/page.tsx`, `tests/unit/batch-membership.test.mjs` | Claude Opus 5 |
| 2026-09-17 | M1-08…M1-15, M5-04, M5-07 | **First live run — the whole auth and admin chain verified against production data.** The user ran `/setup`, added the scoped R2 token and confirmed the Worker deploys. Read-only REST checks with the secret key then confirmed, in `audit_log` and the tables: `/setup` created the branch "Insignia - KKR" and the Owner; `auth.reset_requested` → `auth.password_reset` (`sessions_revoked: true`) → `auth.sign_in`; a student signing in twice produced a revoke at 07:39:43.215 and a replacement at .416, which is the single-session rule executing; `auth.sign_out` with its session revoked a second earlier; `invite.create` (teacher) → `invite.accept` → `auth.sign_in` as teacher, so the new screen-28a path works end to end; `batch.create` then `batch.update` with `teachers_saved: true`, and a `batch_teachers` row to match. Four accounts now exist (2 Owner, 1 teacher, 1 student), one branch, one batch. **Nothing in this session's five commits is unverified any longer.** | *(verification only — no code changed)* | Claude Opus 5, goverdhan-gaur |
| 2026-09-17 | M5-07 | **Batches are editable, and screen 25's dead links are live.** Every batch name on screen 25 linked to `/admin/batches/[batchId]`, which did not exist — so a batch could not be corrected once created. New `app/admin/batches/[batchId]/page.tsx` (edit form + read-only roster), `components/admin/edit-batch-form.tsx`, `updateBatch` in `lib/batches.ts`, `updateBatchAction`, and `getBatchDetail` in `queries/admin.ts`. `status` joins the editable fields via a pure `validateBatchEdit` that **refuses** an unknown value rather than defaulting to `active` — quietly reactivating an archived batch is the failure mode worth spending a branch on. The centre is deliberately *not* editable: moving a batch between branches would strand its students' RLS visibility, and `updateBatch` reads the batch's own branch so a posted `branchId` cannot move it. Teacher edits diff against the current rows rather than delete-and-reinsert. A batch outside the admin's centre is `notFound`, not "forbidden", so ID-guessing learns nothing. Extracted `refuseUnusableTeachers` and `syncBatchTeachers` as shared helpers. 5 new unit checks (226 total), typecheck, lint, build and bundle scan pass. ✅ **Verified live 2026-09-17**: `batch.update` wrote `teachers_saved: true, teacher_count: 1`, and `batch_teachers` holds the row — the teacher diff ran. ⬜ Batch membership (moving students between batches) is still unbuilt; the roster is read-only. | `src/app/admin/batches/[batchId]/page.tsx`, `src/components/admin/edit-batch-form.tsx`, `src/lib/{batch-input,batches}.ts`, `src/lib/actions/batches.ts`, `src/lib/queries/admin.ts`, `src/lib/view-models/admin.ts`, `tests/unit/batches.test.mjs` | Claude Opus 5 |
| 2026-09-17 | M5-07 | **"Create batch" was a 404, and there was no batch write at all.** Screen 25's button has pointed at `/admin/batches/new` since the screen was designed, but the page, the Server Action and the database write were all missing — batches could only be created directly in Postgres. New `lib/batch-input.ts` (pure, import-free, unit-tested like `access.ts`), `lib/batches.ts` (`createBatch` through the secret key, since `batches` is `select`-only with no write policy, guarded by `requirePermission("student:manage")` and audited as `batch.create`), `lib/actions/batches.ts`, `app/admin/batches/new/page.tsx` and `components/admin/create-batch-form.tsx`. Teachers are attached at creation because screen 25 flags an unstaffed batch as broken; `batch_teachers` has no role check of its own — its table comment says that belongs in server code, and `createBatch` is now that code, refusing anyone who is not an active teacher or invigilator at the same centre. A `branch`-scoped admin is pinned to their own centre, matching the invitation rule. 7 new unit checks (221 total), typecheck, lint, build and bundle scan pass; `/admin/batches/new` is a registered route and guarded. ✅ **Verified live 2026-09-17**: `invite.create` (role teacher) → `invite.accept` → `auth.sign_in` as teacher, 10 minutes apart. ⬜ `/admin/batches/[batchId]` is still a 404, though screen 25 links every batch name to it — the batch detail/roster screen is unbuilt. | `src/lib/{batch-input,batches}.ts`, `src/lib/actions/batches.ts`, `src/app/admin/batches/{page.tsx,new/page.tsx}`, `src/components/admin/create-batch-form.tsx`, `src/lib/queries/batches.ts`, `tests/unit/batches.test.mjs` | Claude Opus 5 |
| 2026-09-17 | M5-04, M9-03 | **"Invite a colleague" was a 404.** Screen 28's button has pointed at `/admin/users/new` since the screen was designed, but that page was never built, so there was no way to add staff at all — `inviteAction` had handled `teacher`, `invigilator` and `admin` since M1-02; only the screen was missing. New `app/admin/users/new/page.tsx` + `components/admin/invite-colleague-form.tsx`, kept separate from the student form because a colleague has no batch and no plan but does gain a role. The role list is filtered through `canInviteRole`, so an admin is offered Invigilator and Teacher while only the Owner sees Admin, matching the matrix `permissions.test.mjs` already pins. `listBranchOptions` added beside `listBatchOptions`, scoped by RLS the same way. Also fixed: a successful staff invite revalidated `/admin/students`, so the new colleague did not appear on `/admin/users` until a hard refresh. 214 unit checks, typecheck, lint, build and bundle scan pass; `/admin/users/new` is a registered route and guarded. ⚠️ Not verified signed-in (no account until `/setup` runs). ⬜ Screen 28 still has no pending-invitations list, and `revoke`/`resendInvitationAction` remain hardcoded to `student:manage` and `/admin/students`, so a mistyped colleague invite cannot be cancelled from the UI yet. | `src/app/admin/users/new/page.tsx`, `src/components/admin/invite-colleague-form.tsx`, `src/lib/actions/invitations.ts`, `src/lib/queries/{batches.ts,README.md}` | Claude Opus 5 |
| 2026-09-17 | M1-12 | **Log out works.** `signOutAction` shipped with M1 but nothing ever called it: the Profile button still carried the pre-M1 placeholder `onConfirm={() => setOpen(false)}`, which closed the dialog and left the session live — so a student on a shared lab machine stayed signed in. The button moved to `components/auth/log-out-button.tsx`, now calls the action inside a transition and keeps the dialog's spinner up until the redirect lands. Staff had no log-out anywhere, which is the same shared-machine problem, so `StaffSidebar` ends with one. Verified in the build: `signOutAction` is now a registered server reference for the Profile page and every admin and teacher page, where previously it was referenced by nothing. 214 unit checks, typecheck, lint, build and bundle scan pass. ✅ **Verified live 2026-09-17**: `auth.sign_out` at 08:13:09 with the matching `user_sessions` row revoked at 08:13:08. | `src/components/auth/log-out-button.tsx`, `src/components/ui/staff-sidebar.tsx`, `src/app/(student)/profile/page.tsx`, `PROJECT-MEMORY.md` | Claude Opus 5 |
| 2026-09-17 | M1-12, M1-13 | **Session routing, and a revocation bypass closed.** `/` and `/login` now route by identity: the bare URL always enters through the auth flow, and a live session never sees the login form. The edge and the guards share one pure `routeDecision` (`lib/auth/access.ts`) instead of the branch ladder that was inline in `proxy.ts`; a dead session outranks every other outcome, while a *failed* session read is `unverified` and passes, so a database blip cannot sign the institute out. Three real defects fixed: a Supabase JWT with no `insignia_session` cookie counted as live, so deleting one cookie walked around device revocation and the students' one-session rule; a failed `startSession` left a JWT with no revocable session behind it, which sign-in and first-run setup now refuse; and the cookie was always `Secure`, which local HTTP browsers discard, detaching every development login. Verification: 214 unit checks, typecheck, lint, production build and bundle scan pass, **and the built app was run** — `/` → `/login`, `/home` → `/login?next=%2Fhome`, a stray session cookie grants nothing, and `/login?ended=1` renders its banner without looping. | `src/proxy.ts`, `src/lib/auth/{access,session-cookie,sessions,guard,sign-in}.ts`, `src/lib/actions/auth.ts`, `src/app/page.tsx`, `src/app/(auth)/login/page.tsx`, `src/lib/queries/student.ts`, `tests/unit/auth-{access,session-cookie}.test.mjs`, `src/lib/auth/README.md`, `PROJECT-MEMORY.md` | OpenAI Codex, Claude Opus 5 |
| 2026-09-17 | M1-13, M2-02…05, M4, M5, M6, M7-02 | **Removed production dummy data and made route access role-aware.** Deleted `lib/mock/*`; student, admin, teacher and attempt routes now use the signed-in user's RLS-scoped Supabase client. `proxy.ts` classifies protected route families and redirects unauthenticated or wrong-role users, with the same checks repeated in server layouts and permission checks on sensitive staff pages. Assignment eligibility now resolves plan state, windows, attempt limits, unlocks and resumable attempts from database rows. Private Cloudflare R2 content/audio/answer keys are explicitly deferred: player, review and key routes show honest unavailable states instead of fixtures. Verification: 201 unit checks, 300 DB checks, 26/26 policy sweep, typecheck, lint, production build and client-bundle scan pass. | `src/proxy.ts`, `src/lib/auth/{access,guard}.ts`, `src/lib/queries/*`, `src/app/{(student),admin,teacher,attempt}/**`, `tests/unit/auth-access.test.mjs`, `PROJECT-MEMORY.md` | OpenAI Codex |
