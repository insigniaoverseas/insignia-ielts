...
| **Next task** | **M0-05** finish ([`BUILD-STEPS.md`](BUILD-STEPS.md) step 13): CLI installed, linked and types generated — only **`npx supabase db push`** is left, run by the user (§5). It applies `20260915090941_harden_rls_auto_enable.sql`; then confirm the security advisor is clean. After that: M0-06 (identity migration + RLS). No-DB tasks that can run alongside: M0-12 part 1 (R2 buckets), M0-13 (CSP), M0-15 (`question-types.ts`), M0-18 (`scoring.ts`), M0-21 (docs). |
...
| M0-05 Supabase CLI + generated types | in_progress | goverdhan-gaur, Claude | 2026-09-15 | ~~Drizzle setup + `db/schema.ts`~~ superseded 2026-09-15 — no ORM (§4). ✅ CLI 2.117.0 as devDependency; `login`, `init`, `link` run by the user; `npm run db:types` → `src/lib/supabase/database.types.ts`; `db push --dry-run` shows exactly one pending migration. ⬜ `db push` (user) + advisor clean. Clients (`server/client/admin.ts`) are step 21. |
...
**Start with.** Confirm the push happened (`list_migrations` over the MCP shows `20260915090941`), run the security advisor and confirm it's clean, close M0-05, then M0-06.
