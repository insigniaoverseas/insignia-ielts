...
```
Data (server)     @supabase/ssr + supabase-js · types from `supabase gen types` · no ORM
                  Multi-statement writes (submit) and heavy analytics → Postgres functions
                  called via .rpc(), SECURITY INVOKER by default so RLS still applies
...
| **Cloudflare Workers Free** — the account's plan (Q11): 100,000 app requests/day, **resetting 05:30 IST**; 10 ms CPU per request (small tolerance for rare overruns); 64 MiB uncompressed bundle (4.5 MiB today). **Static files don't count.** Durable Objects: 100,000 requests and 100,000 rows written per day. | A fixed 10-second autosave is ~75,000 requests for one 200-student hour. Over the cap, **every page** shows Cloudflare error 1027 until 05:30 IST. | Budget ≈ **120 Worker requests per student per test** → roughly 600–700 student-tests a day with room for everything else. Save on change + a 60 s heartbeat (§7). Images, scripts and styles are served as static assets, never through the Worker — don't widen `run_worker_first`. The Durable Object counter guards only login, PIN, invite and MCP — never autosave. Keep server rendering light on hot paths and measure CPU per route. Workers Paid ($5/month) is the escape hatch if the load test demands it. |
...
| **Abuse / DoS** | Per-route budgets for login, PIN, invite and MCP via a Durable Object counter (counts in memory; storage written only on a lockout — the free plan allows 100,000 rows written a day). Autosave and submit are bounded by attempt state, `revision` and ownership instead — a Durable Object call per autosave would spend the free quota. Cloudflare WAF in front. | `lib/security/rate-limit.ts` | M0, M1 |
...
│  ├─ proxy.ts                       per-request nonce + security headers (Next 16 proxy)
│  ├─ components/
│  │  ├─ ui/                         shadcn — the design system
│  │  └─ player/                     audio, navigator, widgets, containers
│  └─ lib/
│     ├─ supabase/{server,admin,env}.ts  no browser client — all data access is server-side
│     ├─ supabase/database.types.ts  generated by `supabase gen types` — never edited
│     ├─ security/{rate-limit,sanitize,headers,session}.ts  headers.ts done (M0-13)
...
├─ scripts/                         check-client-bundle.mjs (CI), importers later
│  ├─ import-test.ts
│  └─ import-legacy-tests.ts
├─ docs/                             §16
├─ public/                           static assets + _headers
├─ wrangler.jsonc                    worker: insignia-test
├─ open-next.config.ts
├─ next.config.ts
├─ cloudflare-env.d.ts               generated by `npm run cf-typegen`
├─ MVP-1.md                          this file
├─ PROJECT-MEMORY.md                 progress
├─ BUILD-STEPS.md                    ordered walkthrough
└─ CLAUDE.md                         agent entry point
```

### Design tokens — Tailwind v4

From `DESIGN-PROMPT.md` §A2–A4. **Tailwind v4 is CSS-first** — this goes in `src/app/globals.css`, not a `tailwind.config.js`:
...
docs/
  ...
  data-model.md             ERD + every column, kept in step with supabase/migrations/
...
- **Generated types only** for the database — `supabase gen types` after every migration. Never hand-maintained.
...
| M0-05 | Supabase CLI (`init`, `link`, `db push`) + generated types (`lib/supabase/database.types.ts`) | M0-04 |
...
| **Workers free daily cap (100,000) reached** | Every page shows error 1027 until 05:30 IST — possibly mid-test | ~120 requests per student per test; count requests in the load test (M9-05); watch the daily count in the Cloudflare dashboard; Workers Paid ($5/month) if real usage approaches the cap |
| Power or browser crash mid-mock | Lost attempt, angry student | Save-on-change autosave + 60 s heartbeat + server-held clock. **Build in M2, not M9** — power cuts in Indian labs are a *when*, not an *if* |
